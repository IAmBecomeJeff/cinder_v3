...
void fireworks_up(bool old) {
	if (old) {
		...
		if (old_gravs[0].vel_old <= 0) old_fireworks_status = 2;
		...
		if (my_gravs[0].vel_old <= 0) fireworks_status = 2;
...
void fireworks_explode(bool old) {
	if (old) {
		if (old_my_count++ >= max_count) old_fireworks_status = 0;
		...
		for (int k = 1; k < num_gravs; k++) {
			...
			if (old_gravs[k].distance < 0) { old_gravs[k].fireworks_bright = 0; }
		...
		if (my_count++ >= max_count) fireworks_status = 0;
		...
		for (int k = 1; k < num_gravs; k++) {
			...
			if (my_gravs[k].distance < 0) { my_gravs[k].fireworks_bright = 0; }
