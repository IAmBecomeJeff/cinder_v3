...
//			uint8_t old_bri = old_noise;
//
//			ringPalette(1, i, old_palette, old_index, old_bri, LINEARBLEND);
//		}
//	}else{
//		int scale = 1000;                               // the "zoom factor" for the noise
//
...
//			uint8_t bri = noise;
//
//			ringPalette(0, i, current_palette, index, bri, LINEARBLEND);
//		}
//	}
...
void circnoise_pal_2_ring(bool old) {
	...
	if (old) {
		...
		for (uint8_t i = 0; i < STRIP_LENGTH; i++) {
			...
			ringPalette(1, i, old_palette, old_index, old_bri, LINEARBLEND);				// With that value, look up the 8 bit colour palette value and assign it to the current LED.
		...
		for (uint8_t i = 0; i < STRIP_LENGTH; i++) {
			...
			ringPalette(0, i, current_palette, index, bri, LINEARBLEND);				// With that value, look up the 8 bit colour palette value and assign it to the current LED.
